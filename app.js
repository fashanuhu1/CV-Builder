...
function addReferees(){
    let addReferees = document.getElementById('addReferees');
    let refereesHide = document.getElementById('referees_hide');
    if(referees_count<3)
    {
        ++referees_count;
        var field = 
        `<div class="mb-3">
            ...
            <input type="text" name="referees${referees_count}" class="form-control">
        </div>
        <div class="mb-3">
            <label for="exampleInputEmail1" class="form-label">Tel</label>
            <input type="text" name="tel${referees_count}" class="form-control">
        </div>`;
        var htmlObject = document.createElement('div');
        htmlObject.innerHTML=field;
        addReferees.insertAdjacentElement("beforeend", htmlObject);
    }
    if(referees_count==3)
    {
        refereesHide.style = "display:none";
    }
}